...
本项目优先使用学校官网、就业网、就业质量报告、企业官方招聘站和许可证清楚的开源数据。第三方平台岗位数据只在授权、用户本地导入或聚合后使用，不直接发布来路不明的原始岗位明细。
...
- 本轮接入：从 `school_job.csv` 先接入北京大学、上海交通大学、复旦大学、南京大学、中国科学技术大学、华中科技大学 6 所学校的历史就业去向、升学、出国和主要去向单位。
...
- 当前处理：只作为历史岗位能力词参考，不导入原始 BOSS 岗位明细。
- 原因：BOSS 直聘属于第三方招聘平台，直接抓取和再发布岗位明细存在合规风险。后续可做本地导入器，让用户导入自己有权使用的导出文件，或只保留聚合后的能力词频。
