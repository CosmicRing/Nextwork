...
学校数据策略是先展示官方证据层，再展示专业级字段。当前清华大学、郑州轻工业大学、浙江大学、电子科技大学、西安电子科技大学已有官方或半官方证据样本；同时接入了 `laofeijio2020ojbk2022/gaokaoweb` 的 Apache-2.0 开源历史就业去向数据，已补充 20 所学校的总体去向、升学、出国和主要去向单位。MIT 的 `PotoYang/UniversityCareerWebPage` 已解析为 286 个高校就业网入口，并在学校详情页作为“就业网入口候选”展示，其中 13 个重点学校入口已记录一次访问健康状态。本轮继续接入 MIT 的 `Jsoneft/gaokao-zhiyuan` 湖北 2024 本科批聚合信号：18,430 条院校-专业记录、1,032 所院校、663 个专业名称和 1,067 个物理/历史一分一段分数段；接入 CC-BY-4.0 的 `scottli139/beijing-gaokao-scores` 北京 2023-2025 本科普通批投档线聚合信号：3,950 条记录、638 所院校、83 种选科要求；接入 CC0 的 `scottli139/beijing-gaokao-score-segments` 北京 2023-2025 一分一段聚合信号：1,005 行分数分布、176,571 名三年累计考生和 650/600/500 分典型位次点；把 MIT 的 `shengdabai/college-major-selector` 聚合为全国院校专业索引：2,756 所院校、860 个本科专业、31 省志愿规则及院校/专业分布桶；继续把 `iChipOwO/shandong-admission-history-query` 聚合为山东 2023-2025 投档元数据：1,165 所学校元数据、26 个专业方向组、426 所学科评估覆盖；新增 Apache-2.0 的 `ZhixinZhang-12/ShandongGaokao-admin-data` 山东 2020-2024 管理数据聚合信号：45 个 Excel/CSV、41 个 sheet、约 50.9 万行一分一段/录取/招生计划/报考要求统计；新增 MIT 的 `EvanYao826/china-university-admission` 全国高校招生 SQLite 聚合：1,167 所高校、6,392 条 2023-2025 本科录取、31 省、279 城市和 6,209 条 source_url 覆盖；新增 MIT 的 `GTdim7/xuefeng-skill` 历史录取 SQLite 聚合：248,766 条 2024-2025 录取记录、14 省、18,802 个学校名、27,051 个专业名和 rank/score 字段覆盖；新增 MIT 的 `maliangone/zhangxuefeng-skill-ultimate-version` 专业就业蓝皮书与 2025 分数线参考聚合：2024 届本科毕业半年后平均月收入 6,199 元、6 个绿牌专业、5 个红牌专业、31 省分数线覆盖和 10 个首年新高考省份提醒；把 `shengdabai/gaokao-assistant` 聚合为青海 2025 计划标签库：2,875 条院校标签和公民办/211/双一流统计；接入 `fjx13038033078/ruoyi-CERS` 的推荐系统 SQL 样本聚合：377 所高校、64 条专业投档线、29 个省份分布行；再接入 `firmianay/gaokao` 云南历史分段样本：488 行 2014-2017 分数段统计、64 条专业分数线、25 个专业名；新增 `sgblizzard/gaokao-advisor` 审计覆盖信号：2,360 所高校、488,152 行分数段、377,962 行院校录取线、3,298,297 行专业分数线和 4,951,513 行招生计划的质量审计指标；新增 MIT 的 `liuxusummer/gaokao-advisor` 数据中心聚合：2,919 所高校、875 个本科专业目录、10 省市 2023-2025 投档线 139,843 条、位次表 9,478 条和 2024 选科要求 515,684 条；新增 `Rafael-Luo/spider-college` 高校覆盖参考：README 标注 2,651 条高校基本信息、31 个城市和 199 条历年高考分数线。`spider-college` 只接覆盖量，不复制爬虫日志或采集结果。同时登记但暂不导入 `ziqihe10-droid/xuefeng-agent` 的 AGPLv3 大库候选和 `bbhzyq-dotcom/hngaokazhiyuan` 的无许可证 JSON 数据候选。首页 Source Radar 区分已接样本、数据候选、决策参考和合规阻断。没有专业级就业率或平均工资来源时，继续显示待接入。

自我认知层新增 MIT 的 `Zylcyl/gaokao-advisor` ADI 专业路径评估模型：8 道素质问卷、31 个选项、paths/reach/correct/recover 四维乘法、3 个资源层级、4 个专业标杆和 4 个风险分档。它只作为“先知道自己是谁”的模型参考，不当作高校事实库或录取数据。

BOSS 直聘方向已经检索到 MIT/Apache 项目和历史分析数据，包括 `radishT/Job_Analysis`、`AnHuanAo/Python-DataScience-Final-AI_IT_Analysis`、`DavidHLP/BossAnalyze`、`Yuzhii0718/boozp-neo`、`xiewangzhenyan/python-recruitment-analysis`、`poboll/bosszhipin_spider`、`linxkon/BossZhiPin_Spyder`、`lxm909055383/bosszhipin`、`jhcoco/bosszp`、`My0sot1s/boss-crawler`、`maimaigptlink9/boss-career-ops` 和 `XBuilderLAB/cheat-on-skill`，但不把原始 BOSS 岗位明细直接并入前端事实库。本轮已把 `radishT/Job_Analysis` 中 60,033 条历史能力词映射聚合成 7 类技术方向和能力词 Top 列表，作为“岗位能力偏好”的历史信号展示；新增 `AnHuanAo/Python-DataScience-Final-AI_IT_Analysis` 的 AI/IT 市场聚合洞察：2025-2026 全球 AI 薪资、2024-2026 国内岗位画像、架构/AI 工程/基础设施等高壁垒方向；新增 `poboll/bosszhipin_spider` 合并工作簿去标识化统计：15,897 行历史样本、3 个岗位族、18 个城市、182 个区县、16 类学历、14 类经验和 15 个能力词 Top；新增 `linxkon/BossZhiPin_Spyder` 上海 CSV 去标识化聚合：4,197 行 2024-05-02 历史样本、16 个区、207 个地点、4,178 条可解析薪资、30K 薪资中位、6 个岗位族和 12 个关键词桶；`jhcoco/bosszp` 登记为真实 CSV 候选但因无许可证阻断，`My0sot1s/boss-crawler` 登记为 MIT 本地导入器风险边界但因登录/验证码/全城批量采集阻断，`maimaigptlink9/boss-career-ops` 登记为 5 维评分与 Pipeline 架构参考；`python-recruitment-analysis` 登记为薪资预测、岗位匹配和本地导入器字段参考；`XBuilderLAB/cheat-on-skill` 登记为 BOSS human-in-the-loop 本地 adapter、可学性闸门和反诈规则参考。`lxm909055383/bosszhipin` README 描述约 25 万 BOSS 历史样本并带 CSV，但仓库无许可证，当前只登记为待授权候选，不导入 CSV 或 README 结论。`poboll/bosszhipin_spider` 和 `BossZhiPin_Spyder` README 明确是 BOSS 页面自动化采集，因此只保留聚合统计，不提交 Excel/CSV 原表、公司名、详情网址、岗位明细或采集脚本。后续明细数据只做用户本地导入自己有权使用的导出文件。无许可证的投档线/学校信息仓库先列为待授权候选，不直接复制数据。数据源筛选记录见 [docs/data-sources.md](docs/data-sources.md)。
